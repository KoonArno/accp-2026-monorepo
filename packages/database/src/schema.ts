...
// 1. ENUMS
// --------------------------------------------------------------------------
export const userRoleEnum = pgEnum('user_role', ['admin', 'thstd', 'interstd', 'thpro', 'interpro']);
export const accountStatusEnum = pgEnum('account_status', ['pending_approval', 'active', 'rejected']);
export const eventStatusEnum = pgEnum('event_status', ['draft', 'published', 'cancelled', 'completed']);
export const eventTypeEnum = pgEnum('event_type', ['single_room', 'multi_session']);
export const ticketCategoryEnum = pgEnum('ticket_category', ['primary', 'addon']);
...
export const abstractCategoryEnum = pgEnum('abstract_category', ['clinical', 'social', 'experimental', 'education', 'other']);
export const presentationTypeEnum = pgEnum('presentation_type', ['oral', 'poster']);
export const abstractStatusEnum = pgEnum('abstract_status', ['pending', 'accepted', 'rejected']);
export const speakerTypeEnum = pgEnum('speaker_type', ['keynote', 'panelist', 'moderator', 'guest']);
...
export const eventSpeakers = pgTable('event_speakers', {
    id: serial('id').primaryKey(),
    eventId: integer('event_id').notNull().references(() => events.id),
    speakerId: integer('speaker_id').notNull().references(() => speakers.id),
    sessionId: integer('session_id').references(() => sessions.id),
...
export const abstractCoAuthors = pgTable('abstract_co_authors', {
    id: serial('id').primaryKey(),
    abstractId: integer('abstract_id').notNull().references(() => abstracts.id, { onDelete: 'cascade' }),
    firstName: varchar('first_name', { length: 100 }).notNull(),
    lastName: varchar('last_name', { length: 100 }).notNull(),
    email: varchar('email', { length: 255 }).notNull(),
    institution: varchar('institution', { length: 255 }),
...
export const abstractReviews = pgTable('abstract_reviews', {
    id: serial('id').primaryKey(),
    abstractId: integer('abstract_id').notNull().references(() => abstracts.id),
